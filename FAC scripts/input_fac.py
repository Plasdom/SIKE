...
class LNJLevel:
    def __init__(self, id, element, nuc_chg, num_el, config, config_full, energy, n, l, j, stat_weight):
        self.id = id
...
class LNExTrans(LNTrans):
    def __init__(self, delta_E, lnj_transitions):
        ...
        from_nlj_ids = list(dict.fromkeys(
            [t.from_id for t in lnj_transitions]))
...
class LNJExTrans(LNJTrans):
    def __init__(self, element, from_id, to_id, delta_E, E_grid, sigma, from_stat_weight=None, born_bethe_coeffs=None):
        type = 'excitation'
    ...
    def process_cross_section(self, new_E_grid):
        interp_func = interpolate.interp1d(self.E_grid, np.log(self.sigma), kind='linear',
        ...
        sigma_new[np.where(new_E_grid > bb_thresh)] = 1.1969e-15 * (
            1 / (self.from_stat_weight * E_grid_bb)) * (b0 * np.log(E_grid_bb / self.delta_E) + b1)
...
class LNJIzTrans(LNJTrans):
    def __init__(self, element, from_id, to_id, delta_E, E_grid, sigma, from_stat_weight=None, fit_params=None):
        type = 'ionization'
        LNJTrans.__init__(self, type, element, from_id, to_id, delta_E)
        self.E_grid = E_grid
        self.sigma = sigma
    ...
    def process_cross_section(self, new_E_grid):
        ...
        y = 1 - (1/x)
        sigma_new[np.where(new_E_grid > self.E_grid[-1])] = 1.1969e-15 * (
            1 / (np.pi * self.from_stat_weight * E_grid_fit)) * (p[0] * np.log(x) + p[1] * y**2 + p[2] * (y/x) + p[3] * (y/x**2))
...
class LNJRRTrans(LNJTrans):
    def __init__(self, element, from_id, to_id, delta_E, E_grid, sigma, from_stat_weight=None, to_stat_weight=None, l=None, fit_params=None):
    ...
    def process_cross_section(self, new_E_grid):
        ...
        dgf_dE = (w / (E_grid_fit +
                       p[3])) * p[0] * x ** (-3.5 - self.l + (p[1]/2)) * y ** p[1]
        alpha = 1/137
        g_i = self.to_stat_weight
        g_f = self.from_stat_weight
        sigma_pi = (2 * np.pi * alpha / g_i) * ((1 + alpha**2 *
                                                 eps) / (1 + 0.5 * alpha**2 * eps)) * dgf_dE

        arb_const = 4e-20  # TODO: This constant works, but not sure exactly what it
        sigma_new[np.where(new_E_grid > self.E_grid[-1])] = (alpha**2 / 2) * (g_i / g_f) * (
            w**2 / (eps * (1.0 + 0.5 * alpha**2 * eps))) * sigma_pi * arb_const
...
def get_levels(lev_f):
    ...
    for i, block_start in enumerate(lev_block_idx):
        for l in lines[block_start+2:block_start+2+nlevs[i]]:
            ...
            config = dat[7]
            config_full = dat[8]
            levels.append(LNJLevel(id, element, nuc_chg, num_el, config,
                          config_full, energy, n, l, j, stat_weight))
...
def get_ex_cross_sections(ce_f):
    ...
    for i, block_start in enumerate(trans_block_idx):
        ...
        for j in range(ntrans[i]):
            ...
            trans_lines = block_lines[(
                j * (num_E + 2))+2:(j+1)*(num_E+2)]
            E_grid, _, cross_section = np.loadtxt(trans_lines, unpack=True)

            E_grid += delta_E
            cross_section *= 1e-20
            ex_transitions.append(LNJExTrans(element, from_id, to_id, delta_E, E_grid,
                                  cross_section, from_stat_weight, born_bethe_coeffs=born_bethe_coeffs))
...
def get_iz_cross_sections(ci_f):
    ...
    for i, block_start in enumerate(trans_block_idx):
        ...
        for j in range(ntrans[i]):
            ...
            from_stat_weight = int(trans_dat[1]) + 1
            fit_params = [
                float(p) for p in block_lines[j * (num_E + 2) + 1].split()]
            ...
            E_grid += delta_E
            cross_section *= 1e-20
            iz_transitions.append(LNJIzTrans(element, from_id, to_id, delta_E,
                                  E_grid, cross_section, from_stat_weight, fit_params=fit_params))
...
def get_rr_cross_sections(ci_f):
    ...
    for i, block_start in enumerate(trans_block_idx):
        ...
        for j in range(ntrans[i]):
            ...
            cross_section *= 1e-20
            rr_transitions.append(LNJRRTrans(element, from_id, to_id, delta_E,
                                  E_grid, cross_section, from_stat_weight=from_stat_weight, to_stat_weight=to_stat_weight, l=l, fit_params=fit_params))
...
def get_ai_rates(ai_f):
    ...
    for i, block_start in enumerate(trans_block_idx):
        ...
        for j in range(ntrans[i]):
            ...
            ai_transitions.append(
                LNJAiTrans(element, from_id, to_id, delta_E, rate))
...
def aggregate_states(lnj_levels):
    ...
    aggregated_states = []
    unique_nls = list(dict.fromkeys(
        [(s.n, s.l, s.num_el, s.config) for s in lnj_levels]))
...
def aggregate_transitions(nl_levels, nlj_levels, transitions):
            ...
            for t in type_transitions:
                ...
                if cur_nl_trans_id == nl_trans_id:
                    if nl_trans_id[0] == nl_trans_id[1]:
                        ...
                        delta_E = abs(nl_levels[t.to_nl_id].energy -
            ...
            if len(nl_transitions) > 0:
                ...
                if trans_type == 'ionization':
                    aggregated_transitions.append(
                        LNIzTrans(delta_E, nl_transitions))
                elif trans_type == 'radiative recombination':
                    aggregated_transitions.append(
                        LNRRTrans(delta_E, nl_transitions))
                elif trans_type == 'autotionization':
                    aggregated_transitions.append(
                        LNAiTrans(delta_E, nl_transitions))
    ...
    for trans_type in trans_types:
        for num_el in range(max_num_el+1):
            ...
            type_transitions = [t for t in transitions if t.type ==
                                trans_type and nlj_levels[t.from_id].num_el == num_el]
            nl_trans_ids = [(t.from_nl_id, t.to_nl_id)
                            for t in type_transitions]
            unique_nl_trans_ids = list(dict.fromkeys(nl_trans_ids))
            num_ids = len(unique_nl_trans_ids)
            for i, nl_trans_id in enumerate(unique_nl_trans_ids):
